...
export default function GuidedTour({ pageKey, steps, run, onClose, stepIndex, callback }: GuidedTourProps) {
  ...
  // Created inside GuidedTour via useMemo to preserve context access to t()
  // while preventing component recreation/unmounting on every render
  const CustomTooltip = useMemo(() => {
    return ({
      ...
      const totalSteps = size || rawSteps?.length || 1;
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-[#F6B7C8]/60 uppercase tracking-widest">
                {t("onboarding.step")} {index + 1} {t("onboarding.of")} {totalSteps}
              </span>
            </div>
            <button
              {...closeProps}
  ...
  const stepsWithDisabledBeacons = useMemo(() => {
    return steps.map((step: any) => ({
      ...step,
      // Map properties to match this specific react-joyride version's typings
      skipBeacon: true,
      disableBeacon: true, // fallback
      overlayClickAction: false,
      disableOverlayClose: true, // fallback
      blockTargetInteraction: false,
      spotlightClicks: true, // fallback
    }));
  }, [steps]);
